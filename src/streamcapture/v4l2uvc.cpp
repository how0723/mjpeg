...
static int init_v4l2(struct vdIn *vd)
{
	...
	if(requested_format_found) {
		...
		printf("  Frame format: %c%c%c%c (%c%c%c%c is not supported by device)\n",
		...
		printf("ERROR: Requested frame format %c%c%c%c is not available and no fallback format was found.\n", FOURCC_ARGS(vd->formatIn));
		return -1;
	}
